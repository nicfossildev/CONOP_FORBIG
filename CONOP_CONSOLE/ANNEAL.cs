        ...
        Label111:
            ...
            for (OUTER = 0; OUTER < (int)COMMOD9.NOUTER; OUTER++)
            {
                ...
                if (COMMOD9.PAUSF != 5)
                {
                    ...
                    {
                        frm.MSG_SB.Length = 0;
                        frm.MSG_SB.Append("Progress - 100%");
#if GUIMODE
                        frm.Invalidate();
                        frm.Update();
#else
                        frm.ClearLineAndOutputMessage(frm.MSG_SB.ToString(), true);
#endif
                    }
                    ...
                    {
                        frm.MSG_SB.Length = 0;
                        frm.MSG_SB.AppendFormat("Progress - {0:0.0}%", ((float)(OUTER + 1) / (float)COMMOD9.NOUTER * 100));
#if GUIMODE
                        frm.Invalidate();
                        frm.Update();
#else
                        frm.ClearLineAndOutputMessage(frm.MSG_SB.ToString(), false);
#endif
                    }
                }
